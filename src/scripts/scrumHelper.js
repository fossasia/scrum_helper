...
function allIncluded(outputTarget = 'email') {
	...
	function writeScrumBody() {
		...
		// --- Customization Template Logic for popup ---
		if (outputTarget === 'popup') {
			chrome.storage.local.get(['customizationTemplate'], function (result) {
				const prefs = result.customizationTemplate || {
					showBlockers: true,
					showTasks: true,
					showPRs: true,
					showIssues: true,
				...
				// Filter and build sections based on prefs
				let lastWeekUl = '<ul>';
				let i;
				// PRs, Issues, Tasks, etc. are in lastWeekArray, reviewedPrsArray
				let lastWeekItems = [];
				if (prefs.showTasks) {
					// Tasks are typically in lastWeekArray (non-PR/Issue items)
					lastWeekItems = lastWeekItems.concat(lastWeekArray.filter(x => x && !x.includes('PR') && !x.includes('Issue')));
				}
				if (prefs.showPRs) {
					lastWeekItems = lastWeekItems.concat(lastWeekArray.filter(x => x && x.includes('PR') && !x.includes('Reviewed')));
				}
				if (prefs.showIssues) {
					lastWeekItems = lastWeekItems.concat(lastWeekArray.filter(x => x && x.includes('Issue')));
				}
				if (prefs.showReviewedPRs) {
					lastWeekItems = lastWeekItems.concat(reviewedPrsArray);
				}
				// No sorting by repo/date
				for (i = 0; i < lastWeekItems.length; i++) lastWeekUl += lastWeekItems[i];
				lastWeekUl += '</ul>';
			...
			return; // Don't run the rest of the function for popup
		}
