...
function allIncluded(outputTarget = 'email') {
				...
				if (platform === 'github') {
					if (platformUsernameLocal) {
						...
						if (outputTarget === 'popup') {
							...
							const ErrMessage =
								browser.i18n.getMessage('usernameRequiredError') || 'Please enter your username to generate a report.';
	...
	function saveToStorage(data, subject = null) {
		...
		return browser.storage.local
			...
			.catch((err) => {
				logError('Storage save failed: ', err);
				return false;
			});
	}
	...
	function loadFromStorage() {
		...
		return getCacheTTL().then((currentTTL) => {
			return browser.storage.local.get('githubCache').then((result) => {
