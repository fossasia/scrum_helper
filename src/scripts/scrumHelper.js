...
document.addEventListener("DOMContentLoaded", () => {
  if (
    !usernameValidationListenerAttached &&
    platformUsernameInp &&
    usernameError
  ) {
    ...
    chrome.storage.local
      ...
      .then((items) => {
        ...
        if (Object.keys(corrections).length > 0) {
          console.warn(
            "[SCRUM-HELPER]: Detected conflicting advanced filters; normalizing stored settings.",
            corrections,
          );
        ...
        if (platform === "github") {
          ...
          if (outputTarget === "popup") {
            const ErrMessage =
              chrome.i18n.getMessage("unknownPlatformError") ||
...
${prs
  .map(
    (
      pr,
      i,
    ) => `	repo${i}: repository(owner: "${pr.owner}\", name: "${pr.repo}\") {
    ...
    try {
      const storageData = await new Promise((resolve) => {
        chrome.storage.local.get(
          ["startingDate", "endingDate", "yesterdayContribution"],
          resolve,
        );
      });
