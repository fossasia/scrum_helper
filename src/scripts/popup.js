...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!repoSearch || !useRepoFilter) {
    ...
    function clearAllRepos() {
      ...
      updateRepoDisplay();
      saveRepoSelection();
      // Refresh dropdown so cleared repos are immediately available again
      if (typeof filterAndDisplayRepos === "function" && repoSearch) {
        const query = repoSearch.value ? repoSearch.value.toLowerCase() : "";
        filterAndDisplayRepos(query);
      }
    }
    ...
    function updateRepoDisplay() {
      if (clearAllReposBtn)
        clearAllReposBtn.classList.toggle("hidden", selectedRepos.length === 0);
    ...
    if (clearAllReposBtn) {
      clearAllReposBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        // Move focus to repo search input before button hides
        if (repoSearch && typeof repoSearch.focus === "function") {
          repoSearch.focus();
        }
        clearAllRepos();
      });
