...
if (organisationNameInput) {
    ...
    organisationNameInput.addEventListener('blur', function () {
        ...
        this.value = orgValue;
        chrome.storage.local.set({ organisationName: orgValue }, function () {
            console.log('Organisation name saved:', orgValue);
            // Clear the cache when org name changes
            chrome.storage.local.remove('githubCache', function () {
                console.log('Cache cleared due to organisation name change');
            });
        });
    });
}