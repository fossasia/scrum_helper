...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Multi-Profile Support Initialization
  if (typeof initializeProfileSupport === "function") {
    setTimeout(initializeProfileSupport, 100);
  }
...
async function initializeProfileSupport() {
  const profileSelect = document.getElementById("profileSelect");
  const addProfileBtn = document.getElementById("addProfile");
  const deleteProfileBtn = document.getElementById("deleteProfile");

  if (!profileSelect || !addProfileBtn || !deleteProfileBtn) return;

  let { profiles, currentProfileId } = await new Promise((resolve) => {
    chrome.storage.local.get(["profiles", "currentProfileId"], resolve);
  });


  if (!profiles || !Array.isArray(profiles) || profiles.length === 0) {
    // Migration: Create first profile from existing settings
    const currentSettings = await new Promise((resolve) => {
      chrome.storage.local.get(PROFILE_KEYS, resolve);
    });

    const defaultProfile = {
      id: "default-" + Date.now(),
      name: chrome.i18n.getMessage("defaultProfileName") || "Default",
      ...currentSettings,
    };
    ...
    await new Promise((resolve) => {
      chrome.storage.local.set({ profiles, currentProfileId }, resolve);
    });
  }
  ...
  function renderProfiles() {
    ...
    profiles.forEach((p) => {
      ...
      option.selected = p.id === currentProfileId;
  ...
  async function switchProfile(id) {
    ...
    // 1. Save current UI/Storage state to the old profile BEFORE switching
    const currentData = await new Promise((resolve) => {
      chrome.storage.local.get(PROFILE_KEYS, resolve);
    });

    profiles = profiles.map((p) => {
      if (p.id === oldProfileId) {
    ...
    // 3. Update root storage with new profile data (for scrumHelper compatibility)
    const newData = {};
    PROFILE_KEYS.forEach((key) => {
      newData[key] = newProfile[key] ?? (key === "selectedRepos" ? [] : "");
    });

    await new Promise((resolve) => {
      chrome.storage.local.set(
        { ...newData, profiles, currentProfileId },
        resolve,
      );
    });

    // 4. Reload page to let existing initialization logic pick up new values
  ...
  addProfileBtn.addEventListener("click", async () => {
    const name = prompt(
      chrome.i18n.getMessage("newProfilePrompt") || "Enter profile name:",
    );
  ...
  deleteProfileBtn.addEventListener("click", async () => {
    ...
    const msg =
      chrome.i18n.getMessage("deleteProfileConfirm") || "Delete profile?";
    if (!confirm(msg.replace("$1", currentProfile.name))) return;

    profiles = profiles.filter((p) => p.id !== currentProfileId);
    currentProfileId = profiles[0].id;

    // Reload to active first profile
    const firstProfile = profiles[0];
    const newData = {};
    PROFILE_KEYS.forEach((key) => {
      newData[key] = firstProfile[key] ?? (key === "selectedRepos" ? [] : "");
    });
...
// Ensure profiles are initialized after DOM is ready
// (Logic integrated into main DOMContentLoaded listener)
