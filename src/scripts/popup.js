...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['darkMode'], (result) => {
        if (result.darkMode) {
            ...
            if (darkModeToggle) darkModeToggle.src = 'icons/light-mode.png';
            if (settingsIcon) {
    ...
    if (toggleTokenBtn && githubTokenInput && tokenEyeIcon) {
    ...
    function initializePopup() {
        ...
        chrome.storage.local.get(
            ...
            (result) => {
                if (result.projectName && projectNameInput) projectNameInput.value = result.projectName;
                if (result.orgName && orgInput) orgInput.value = result.orgName;
                ...
                if (onlyPRsCheckbox && typeof result.onlyPRs !== 'undefined') {
                    onlyPRsCheckbox.checked = result.onlyPRs;
                }
                if (onlyRevPRsCheckbox && typeof result.onlyRevPRs !== 'undefined') {
                ...
                if (onlyIssuesCheckbox && onlyPRsCheckbox && onlyIssuesCheckbox.checked && onlyPRsCheckbox.checked) {
                    onlyPRsCheckbox.checked = false;
                    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
                        chrome.storage.sync.set({ onlyPRs: false });
                    }
                }
                if (result.githubToken && githubTokenInput) githubTokenInput.value = result.githubToken;
                if (result.cacheInput && cacheInput) cacheInput.value = result.cacheInput;
                ...
                if (result.startingDate && startingDateInput) startingDateInput.value = result.startingDate;
                if (result.endingDate && endingDateInput) endingDateInput.value = result.endingDate;

                // Load platform-specific username
                const platform = result.platform || 'github';
                const platformUsernameKey = `${platform}Username`;
                if (platformUsername) platformUsername.value = result[platformUsernameKey] || '';
            },
        );
    ...
    function showReportView() {
        if (!reportSection || !settingsSection) return;
...
function setPlatformDropdown(value) {
    ...
    if (platformSelectHidden) platformSelectHidden.value = value;
...
chrome.storage.local.get(['platform'], (result) => {
    ...
    if (platformSelectHidden) platformSelectHidden.value = platform;
    updatePlatformUI(platform);
});
